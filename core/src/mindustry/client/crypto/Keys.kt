...
class KeyQuad {
    ...
    constructor(input: ByteArray) {
        ...
        xPrivateKey = mindustry.client.crypto.XPrivateKey(buff.bytes(XPrivateKey.SECRET_SIZE), 0)
        xPublicKey = mindustry.client.crypto.XPublicKey(buff.bytes(XPublicKey.KEY_SIZE), 0)
        edPrivateKey = mindustry.client.crypto.EdPrivateKey(buff.bytes(EdPrivateKey.KEY_SIZE), 0)
        edPublicKey = mindustry.client.crypto.EdPublicKey(buff.bytes(EdPublicKey.KEY_SIZE), 0)
    }
...
class PublicKeyPair {
    ...
    constructor(input: ByteArray) {
        ...
        xPublicKey = mindustry.client.crypto.XPublicKey(buf.bytes(XPublicKey.KEY_SIZE), 0)
        edPublicKey = mindustry.client.crypto.EdPublicKey(buf.bytes(EdPublicKey.KEY_SIZE), 0)
    }
